  ...
  <div class="panel-body clearfix">
    ...
    <#if (user.account?has_content)>
      <#if (user.account.id == question.authorId)>
        <a href="${config.url.basedir}/question/edit?id=${question.id}" class="btn btn-default">Edit</a>
      <#else>
...
<p>
  <a href="${config.url.basedir}/answer/edit?questionId=${question.id}" class="btn btn-primary">Post Your Answer</a>
</p>
...
<#assign _returnTo='${"/question/${question.id}?offset=${answers.offset}&limit=${answers.limit}"?url}'>
    ...
    <div class="pull-right">
      <#if (user.account?has_content)>
        <#if (user.account.id == x.authorId)>
          <a href="${config.url.basedir}/answer/edit?id=${x.id}" class="btn btn-default">Edit</a>
        <#else>
        <div class="btn-group">
          <a href="${config.url.basedir}/answer/vote?id=${x.id}&point=1&returnTo=${_returnTo}" class="btn btn-default"><span class="glyphicon glyphicon-arrow-up" title="Up"></span></a>
          <span class="btn btn-default">${x.positivePoints}</span>
          <a href="${config.url.basedir}/answer/vote?id=${x.id}&point=-1&returnTo=${_returnTo}" class="btn btn-default"><span class="glyphicon glyphicon-arrow-down" title="Down"></span></a>
...
<@p.defaultPagination answers "${config.url.basedir}/question/${question.id}"></@p.defaultPagination>
</#if>
